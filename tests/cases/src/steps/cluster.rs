use raft::{
    ClientRequestHandler, NewDataRequest, NodeWorker, PeerRequestChannels, PeerRequestHandler,
};
...
pub fn start_initial_cluster<F, Cc, Pc>(
    nodes: Vec<u64>,
    peer_communicator: Pc,
    node_creator: F,
) -> CaseCluster<Cc, Pc>
where
    F: Fn(u64, Vec<u64>, Pc) -> (NodeWorker, Cc),
    Cc: ClientRequestHandler,
    Pc: PeerRequestHandler + PeerRequestChannels,
{
    ...
    for node_id in all_nodes.clone() {
        let (node_worker, client_request_handler) =
            node_creator(node_id, all_nodes.clone(), peer_communicator.clone());
    ...
    Pc: PeerRequestHandler + PeerRequestChannels,
{
    ...
    where
        F: Fn(u64, Vec<u64>, Pc) -> (NodeWorker, Cc),
    {
        let (node_worker, client_request_handler) = node_creator(
            new_node_id,
            self.initial_nodes.clone(),
            self.peer_communicator.clone(),
        );

        self.node_workers.push(node_worker);
        self.client_handlers
            .insert(new_node_id, client_request_handler);
    }
