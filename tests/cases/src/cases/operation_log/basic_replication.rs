use crate::steps;
use raft::{PeerRequestHandler, PeerRequestChannels, NodeConfiguration, ElectionTimer, NodeState, NodeTimings, OperationLog};
...
pub fn run() {
	...
	let operation_log = MemoryOperationLog::new(ClusterConfiguration::new(cluster.initial_nodes.clone()), tx);
	// run new server
