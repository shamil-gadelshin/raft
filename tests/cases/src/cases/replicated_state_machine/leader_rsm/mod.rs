...
pub fn run() {
	...
	let (tx, rx): (Sender<DataEntryContent>, Receiver<DataEntryContent>) = crossbeam_channel::unbounded();
	let rsm = custom_rsm::MemoryRsm::new( tx);
	let cluster = steps::cluster::start_initial_cluster(node_ids, peer_communicator.clone(), |node_id, all_nodes, peer_communicator| {
		let election_timer = FixedElectionTimer::new(1000);
		let (client_request_handler, node_config) = configuration::create_node_configuration_inproc(node_id, all_nodes, peer_communicator, election_timer, rsm.clone());
