...
where Psd: PeerCommunicatorServiceDiscovery{
	...
	append_entries_channels: Arc<RwLock<HashMap<u64,DuplexChannel<raft::AppendEntriesRequest, raft::AppendEntriesResponse>>>>,
...
impl <Psd> NetworkPeerCommunicator<Psd>
	...
	pub fn new(host : String,
			   ...
			   timeout : Duration,
			   run_server: bool,
			   service_discovery: Psd) -> NetworkPeerCommunicator<Psd> {
...
impl <Psd> server::PeerRequestHandler for NetworkPeerCommunicator<Psd>
	...
	fn append_entries(&mut self, request: Request<AppendEntriesRequest>) -> Self::AppendEntriesFuture {
		...
		let send_result = self.append_entries_request_tx(self.node_id).send_timeout(raft_request, self.timeout);
		if let Err(err) = send_result {
			return future::err(tower_grpc::Status::new(tower_grpc::Code::Unknown, format!(" error:{}",err)))
		}

		let receive_result = self.append_entries_response_rx(self.node_id).recv_timeout(self.timeout);
	...
	fn request_vote(&mut self, request: Request<VoteRequest>) -> Self::RequestVoteFuture {
		...
		let send_result = self.vote_request_tx(self.node_id).send_timeout(raft_request, self.timeout);
		if let Err(err) = send_result {
			return future::err(tower_grpc::Status::new(tower_grpc::Code::Unknown, format!(" error:{}",err)))
		}

		let receive_result = self.vote_response_rx(self.node_id).recv_timeout(self.timeout);
		if let Err(err) = receive_result {
			return future::err(tower_grpc::Status::new(tower_grpc::Code::Unknown, format!(" error:{}",err)))
		}
