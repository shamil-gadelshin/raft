...
pub mod client {
    ...
    impl<T> PeerRequestHandler<T> {
        ...
        /// Peer request service: Append Entries RPC and Vote Request RPC
        pub fn append_entries<R>(&mut self, request: grpc::Request<AppendEntriesRequest>) -> grpc::unary::ResponseFuture<AppendEntriesResponse, T::Future, T::ResponseBody>
        where T: grpc::GrpcService<R>,
              grpc::unary::Once<AppendEntriesRequest>: grpc::Encodable<R>,
        {
            let path = http::PathAndQuery::from_static("/grpc_peer_communicator.PeerRequestHandler/AppendEntries");
            self.inner.unary(request, path)
        }

        /// Peer request service: Append Entries RPC and Vote Request RPC
        pub fn request_vote<R>(&mut self, request: grpc::Request<VoteRequest>) -> grpc::unary::ResponseFuture<VoteResponse, T::Future, T::ResponseBody>
        where T: grpc::GrpcService<R>,
              grpc::unary::Once<VoteRequest>: grpc::Encodable<R>,
        {
            let path = http::PathAndQuery::from_static("/grpc_peer_communicator.PeerRequestHandler/RequestVote");
            self.inner.unary(request, path)
        }
    }
}
...
pub mod server {
    ...
    pub trait PeerRequestHandler: Clone {
        type AppendEntriesFuture: futures::Future<Item = grpc::Response<AppendEntriesResponse>, Error = grpc::Status>;
        type RequestVoteFuture: futures::Future<Item = grpc::Response<VoteResponse>, Error = grpc::Status>;

        /// append entry request to the peer
        fn append_entries(&mut self, request: grpc::Request<AppendEntriesRequest>) -> Self::AppendEntriesFuture;
    ...
    where T: PeerRequestHandler,
    {
        pub fn new(peer_request_handler: T) -> Self {
            Self { peer_request_handler }
        }
    }
        ...
        fn call(&mut self, request: http::Request<grpc::BoxBody>) -> Self::Future {
            ...
            match request.uri().path() {
                "/grpc_peer_communicator.PeerRequestHandler/AppendEntries" => {
                    let service = peer_request_handler::methods::AppendEntries(self.peer_request_handler.clone());
                    let response = grpc::unary(service, request);
                    peer_request_handler::ResponseFuture { kind: AppendEntries(response) }
                }
                "/grpc_peer_communicator.PeerRequestHandler/RequestVote" => {
                    let service = peer_request_handler::methods::RequestVote(self.peer_request_handler.clone());
                    let response = grpc::unary(service, request);
                    peer_request_handler::ResponseFuture { kind: RequestVote(response) }
                }
                _ => {
                    peer_request_handler::ResponseFuture { kind: __Generated__Unimplemented(grpc::unimplemented(format!("unknown service: {:?}", request.uri().path()))) }
                }
            }
        }
    }
    ...
    pub mod peer_request_handler {
        ...
        where T: PeerRequestHandler,
        {
            pub(super) kind: Kind<
                // AppendEntries
                grpc::unary::ResponseFuture<methods::AppendEntries<T>, grpc::BoxBody, AppendEntriesRequest>,
                // RequestVote
                ...
                match self.kind {
                    AppendEntries(ref mut fut) => {
                        ...
                        let response = response.map(|body| {
                            ResponseBody { kind: AppendEntries(body) }
                        });
                        Ok(response.into())
                    }
                    RequestVote(ref mut fut) => {
                        let response = try_ready!(fut.poll());
                        let response = response.map(|body| {
                            ResponseBody { kind: RequestVote(body) }
                        });
                        Ok(response.into())
                    }
        ...
        where T: PeerRequestHandler,
        {
            pub(super) kind: Kind<
                // AppendEntries
                grpc::Encode<grpc::unary::Once<<methods::AppendEntries<T> as grpc::UnaryService<AppendEntriesRequest>>::Response>>,
                // RequestVote
                grpc::Encode<grpc::unary::Once<<methods::RequestVote<T> as grpc::UnaryService<VoteRequest>>::Response>>,
                // A generated catch-all for unimplemented service calls
                (),
            >,
        }
        ...
        pub mod methods {
            use ::tower_grpc::codegen::server::*;
            use super::super::{PeerRequestHandler, AppendEntriesRequest, AppendEntriesResponse, VoteRequest, VoteResponse};
