...
impl server::ClientRequestHandler for NetworkClientCommunicator {
    ...
    fn add_server(&mut self, request: Request<AddServerRequest>) -> Self::AddServerFuture {
        trace!("Add server request {:?}", request);
        let raft_req = raft::AddServerRequest {
            new_server: request.into_inner().new_server,
        };
        let send_result = self
            .add_server_duplex_channel
            .request_tx
            .send_timeout(raft_req, self.timeout);
    ...
    fn new_data(&mut self, request: Request<NewDataRequest>) -> Self::NewDataFuture {
        ...
        let raft_req = raft::NewDataRequest {
            data: Arc::new(Box::leak(data)),
        };
        let send_result = self
            .new_data_duplex_channel
            .request_tx
            .send_timeout(raft_req, self.timeout);
        if let Err(err) = send_result {
            return future::err(tower_grpc::Status::new(
