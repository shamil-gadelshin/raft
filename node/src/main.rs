...
use raft::{NodeLimits, NodeState};

use raft_modules::MemoryOperationLog;
use raft_modules::NetworkClientCommunicator;
use raft_modules::{ClusterConfiguration, MemoryRsm, MockNodeStateSaver, RandomizedElectionTimer};
...
fn main() {
    ...
    let node_config = NodeConfiguration {
        ...
        client_communicator: client_request_handler,
        election_timer: RandomizedElectionTimer::new(1000, 4000),
        limits: NodeLimits::default(),
        operation_log: MemoryOperationLog::new(cluster_configuration),
        rsm: MemoryRsm::new(),
...
fn get_network_peer_communicator(
    node_id: u64,
) -> NetworkPeerCommunicator<StaticTableServiceDiscovery> {
