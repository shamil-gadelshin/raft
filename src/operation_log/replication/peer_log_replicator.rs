...
pub fn replicate_log_to_peer<Log, Rsm, Pc, Ns, Cl>(
    params: LogReplicatorParams<Log, Rsm, Pc, Ns, Cl>,
...
fn process_replication_request<Log, Rsm, Pc, Ns, Cl>(
    params: &LogReplicatorParams<Log, Rsm, Pc, Ns, Cl>,
    peer_id: u64,
) where
    Log: OperationLog,
    ...
    Cl: Cluster,
{
    ...
    let resp_result = params
        .communicator
        .send_append_entries_request(peer_id, append_entries_request);
    match resp_result {
        Ok(resp) => {
            if resp.success {
                ...
                if next_index > 1 {
                    ...
                    warn!(
                        "Unsuccessful replicate log request and next_index <= 1  - Node {} to peer {} ",
                        node.id, peer_id);
                }
                ...
                params
                    ...
                    .send(peer_id)
                    .expect("can send update peer log request")
            }
        }
        Err(err) => {
