    ...
    Cl: Cluster,
{
    ...
    pub fn append_content_to_log(&mut self, content: EntryContent) -> Result<bool, RaftError> {
        ...
        let add_entry_to_log_result = self.append_entry_to_log(entry.clone());
        if let Err(err) = add_entry_to_log_result {
        ...
        if let Err(err) = add_entry_to_rsm_result {
            ...
            error!("{}", msg);
            return new_err("Cannot apply entry to rsm".to_string(), msg);
        }
