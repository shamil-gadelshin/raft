...
mod tests {
    ...
    use crate::{ReplicatedStateMachine, LogEntry, RaftError, Cluster, NodeStateSaver, NodeState, PeerRequestHandler, VoteRequest, VoteResponse, AppendEntriesRequest, AppendEntriesResponse, OperationLog, EntryContent, DataEntryContent};
    ...
    fn test_prev_term_index_with_single_record() {
        ...
        log.append_entry(LogEntry{
            index : 1,
            term : 1,
            entry_content: EntryContent::Data(DataEntryContent{data: Arc::new(b"some")})
        });
